...
s_36_parent_ids = np.array([0, 0, 1, 2, 0, 4, 5, 0, 7, 8, 9, 8, 11, 12, 8, 14, 15], dtype=np.int)
...
def calc_total_skeleton_length_bone(joints, total=True):
    length = []
    for jt_a, jt_b in s_36_bone_jts:
        l = np.linalg.norm(joints[jt_a] - joints[jt_b])
        length.append(l)

    return sum(length) if total is True else length
