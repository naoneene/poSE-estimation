...
POSE_RESNET = edict()
POSE_RESNET.NET_TYPE = 'ResNet50' # 'ResNet18''ResNet34''ResNet50''ResNeXt50''ResNeXt101'
...
MODEL_EXTRAS = {
        'posenet': POSE_RESNET,
        'poseSEnet': POSE_RESNET,
}

# common params for NETWORK
config.MODEL = edict()
config.MODEL.NAME = 'posenet'
config.MODEL.INIT_WEIGHTS = True
config.MODEL.PRETRAINED = ''
config.MODEL.RESUME = ''
config.MODEL.RESUME_GCN = ''
config.MODEL.RESUME_REF = ''
...
# H3.6M specific training params
...
config.DATASET.VOC = '' # path to PASCAL VOC2012 dataset
...
def get_model_name(cfg):
	...
	if name == 'posenet' or name == 'poseSEnet' or 'pose2net':
		name = '{model}_{net_type}'.format(
			model=name,
			net_type=extra.NET_TYPE)
