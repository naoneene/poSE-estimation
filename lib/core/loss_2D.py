...
def weighted_ruber_loss(input, target, weights, size_average, norm=False):
    ...
    diff = input - target
    abs = torch.abs(diff)
    out = torch.where(abs<c, abs, torch.sqrt(2*c*abs-c**2))
    #print(out[0])
...
def softmax_integral_tensor(preds, num_joints, output_3d, hm_width, hm_height, hm_depth):
    ...
    if output_3d:
        x, y = generate_2d_integral_preds_tensor(preds, num_joints, hm_width, hm_height, hm_depth)
    else:
...
class L1JointLocationLoss(nn.Module):
    ...
    def forward(self, preds, *args):
        ...
        pred_jts = softmax_integral_tensor(preds, self.num_joints, True, hm_width, hm_height)
...
class RuberJointLocationLoss(nn.Module):
    def __init__(self, num_joints, size_average=True, reduce=True, norm=False):
        super(RuberJointLocationLoss, self).__init__()
        self.size_average = size_average
        self.reduce = reduce
        self.num_joints = num_joints
        self.norm = norm
    ...
    def forward(self, preds, *args):
        ...
        _assert_no_grad(gt_joints)
        _assert_no_grad(gt_joints_vis)
        return weighted_ruber_loss(pred_jts, gt_joints, gt_joints_vis, self.size_average, self.norm)
