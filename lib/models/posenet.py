...
class PoseNet(nn.Module):
    ...
    def __init__(self, block, layers, cfg, groups=1, width_per_group=64):
...
net_spec = {'ResNet18':     (BasicBlock, [2, 2, 2, 2], 1, 64),
            'ResNet34':     (BasicBlock, [3, 4, 6, 3], 1, 64),
            'ResNet50':     (Bottleneck, [3, 4, 6, 3], 1, 64),
            'ResNet101':    (Bottleneck, [3, 4, 23, 3], 1, 64),
            'ResNeXt50':    (Bottleneck, [3, 4, 6, 3], 32, 4),  #resnext50_32x4d
            'ResNeXt101':   (Bottleneck, [3, 4, 23, 3], 64, 4)  #resnext101_64x4d
            }

def get_pose_net(cfg, is_train, **kwargs):
    net_type = cfg.MODEL.EXTRA.NET_TYPE

    block_class, layers, kwargs['groups'], kwargs['width_per_group'] = net_spec[net_type]

    model = PoseNet(block_class, layers, cfg, **kwargs)
